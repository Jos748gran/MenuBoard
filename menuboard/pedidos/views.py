...
from pedidos.forms import CustomLoginForm, CustomUserCreationForm, MesaForm, ClienteForm, PedidoForm, MeseroForm, \
    PersonalCocinaForm, ItemPedidoForm
...
def crear_item_pedido(request):
    ...
    if request.user.groups.filter(name='Clientes').exists():
        return render(request, 'objetos_item_pedido/crear_item_pedido.html', data)
...
def modificar_item_pedido(request, id):
    ...
    return render(request, 'objetos_item_pedido/modificar_item_pedido.html', {'form': form})
...
def eliminar_item_pedido(request, id):
    ...
    if request.method == 'POST':
        item_pedido.delete()
        return redirect('pedidos')
    return render(request, 'objetos_item_pedido/eliminar_item_pedido.html', {'item_pedido': item_pedido})
